name: Build Sliver Server static for CentOS 5.11 i686
      ...
      - name: Fix compilation errors
        run: |
          ...
          if [ -f ./server/gogo/go.go ]; then
            # 替换没有参数的 GetZigDir 调用，添加 GOOS 和 GOARCH 参数
            sed -i 's/GetZigDir()/GetZigDir("linux", "amd64")/g' ./server/gogo/go.go
