    ...
    steps:
      ...
      - name: Create Dockerfile for Alpine
        run: |
          ...
          # 安装必要的Go工具
          RUN go install -v github.com/jessevdk/go-assets-builder@latest
      ...
      - name: Build Sliver for CentOS 5.11
        run: |
          ...
          find ./server/assets -type d | sort
          
          # 显示当前工作目录下文件
          echo "Asset files:"
          find ./server/assets/fs -type f | sort
          
          # 生成go-assets.go文件
          echo "Generating go-assets.go..."
          cd server/assets
          go-assets-builder -p assets -o assets_generated.go fs/
          
          # 添加文件内容
          cat > assets.go << 'ASSETEND'
          //go:build !generate
          // +build !generate

          // This file loads all the embedded assets
          ...
          // Setup initializes the embedded assets
          func Setup() {
          	fmt.Printf("[setup] Embedded assets loaded\n")
          }
          ASSETEND
